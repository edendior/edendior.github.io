//ADDS TYPING TEXT AS TITLE 
window.addEventListener('DOMContentLoaded', () => {
    const text = "Eden's Portfolio";
    const target = document.querySelector('.pink-text');
    target.textContent = ""; // Clears existing text

    let i = 0;
    function typeWriter() {
      if (i < text.length) { //i = 0, if the text length is greater than 0, run
        target.textContent += text.charAt(i);
        i++;
        setTimeout(typeWriter, 180); // this is the speed of typing
      }
    }
